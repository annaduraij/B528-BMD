...
class DataIO:
    ...
    def df_to_file(
        ...
        if file_format == 'xlsx' or file_format == 'xls':
            df.to_excel(file_path, index=keep_index, header=keep_header)
        elif file_format == 'csv' or file_format == 'txt':
            df.to_csv(file_path, index=keep_index, header=keep_header, sep=',')
